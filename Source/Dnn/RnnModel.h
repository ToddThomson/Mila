...
#ifndef MILA_DNN_RNN_MODEL_H_
#define MILA_DNN_RNN_MODEL_H_
...
#include "Dropout.h"
#include "RnnDataSet.h"
#include "RnnOperation.h"

#include "DnnModel.h"
#include "DnnModelBuilder.h"
#include "DnnModelOptions.h"
#include "RnnModelOptions.h"
#include "RnnLayerCollection.h"
...
namespace Mila::Dnn
{
    ...
    private:
        ...
        void CalculateParameterSizes()
        {
            ...
            inputTensorSize_ = options_.seqLength * options_.batchSize * options_.inputSize;
            outputTensorSize_ = options_.seqLength * options_.batchSize * options_.hiddenSize * bidirectionalScale_;
        ...
        void CreateInputOutputDataSets( DnnModelBuilder builder )
        {
            ...
            {
                seqLengthArray_.push_back( options_.seqLength );
            }
            ...
            x.SetDataType( dataType )
                ...
                .SetMaxSeqLength( options_.seqLength )
                .SetBatchSize( options_.batchSize )
                .SetVectorSize( options_.vectorSize )
                .SetSeqLengthArray( seqLengthArray_ )
        ...
        void SetupMemorySpace( DnnModelBuilder builder )
        {
            ...
            {
                throw cudnnException( "Failed to get RNN WeightBias space size.", status );
            }
            ...
            if ( status != CUDNN_STATUS_SUCCESS )
            {
                throw cudnnException( "Failed to get workspace/reserve space size", status );
            }
            ...
            reserveSpace_ = CudaMemory( reserveSpaceSize_ );
        ...
        void CreateLayers( DnnModelBuilder builder )
        {
            ...
            for ( int layerId = 0; layerId < options_.numLayers * bidirectionalScale_; layerId++ )
            {
                for ( int linLayerId = 0; linLayerId < linLayers; linLayerId++ )
                {
                    ...
                    if ( status != CUDNN_STATUS_SUCCESS )
                    {
                        throw cudnnException( "Failed to get RNN Weight Params", status );
                    }
                    ...
                    if ( weightMatrixAddress )
                    {
                        ...
                        if ( status != CUDNN_STATUS_SUCCESS )
                        {
                            throw cudnnException( "Failed to get TensorNdDescriptor for weight states.", status );
                        }
    ...
    private:
            ...
            CudaMemory& c )
        {
                ...
                for ( int m = 0; m < options_.batchSize; m++ )
                {
                    ...
                    for ( int j = 0; j < options_.seqLength; j++ )
                    {
            ...
            CudaMemory& cy_data )
        {
            ...
            if ( status != CUDNN_STATUS_SUCCESS )
            {
                std::cout << "CudnnForward() failed: " << CuDNN::to_string( status) << std::endl;

                throw cudnnException( "Failed CudnnForward() call", status );
            }
            ...
            CudaMemory& cx_data, CudaMemory& dcy_data, CudaMemory& dcx_data )
        {
            ...
            if ( status != CUDNN_STATUS_SUCCESS )
            {
                ...
                throw cudnnException( "Failed CudnnBackwardData_v8()", status );
            }
