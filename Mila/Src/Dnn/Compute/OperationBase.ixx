...
*/  
export  
template <typename TInput, typename TCompute, typename MR>
...
public:  
	...
	*/  
	virtual void forward(  
		...
		std::vector<std::shared_ptr<Tensor<TCompute, MR>>>& output_cache ) const = 0;  
	...
	*/  
	virtual void backward(  
		...
		const std::vector<std::shared_ptr<Tensor<TCompute,MR>>>& parameters,  
		std::vector<std::shared_ptr<Tensor<TCompute,MR>>>& outputGrads ) const {  
