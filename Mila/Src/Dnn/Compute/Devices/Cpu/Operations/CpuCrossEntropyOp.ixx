...
namespace Mila::Dnn::Compute
{
    ...
    public:
        ...
        CpuCrossEntropyOp( const CrossEntropyConfig& config )
            : OperationBase( OperationType::CrossEntropyOp ), config_( config ) {
        }
        ...
        void backward(
            ...
            if ( parameter_gradients.size() > 0 ) {
                ...
                auto T = input.shape()[ 1 ];
                auto Vp = parameters[ 0 ]->shape()[ 2 ];
                auto V = config_.getVocabSize(); // attributes.get<int>( "vocab_size", Vp ); // Actual vocabulary size (without padding)
        ...
        private:
            CrossEntropyConfig config_; ///< Configuration for the CrossEntropy operation.
    };
