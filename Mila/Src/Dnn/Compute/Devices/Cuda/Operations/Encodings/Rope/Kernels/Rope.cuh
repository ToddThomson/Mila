/**
 * @file Rope.cuh
 * @brief CUDA kernel declarations for RoPE encoder forward/backward launchers.
 */

#pragma once

#include <cuda_runtime.h>
#include <cuda_fp16.h>

namespace Mila::Dnn::Compute::Cuda::Rope
{
    // FP32 host-launchable kernels
    void cuda_rope_forward_fp32(
        float* Y,
        const int* X,
        const float* wte, const float* wpe,
        int B, int T, int C,
        cudaStream_t stream );

    void cuda_rope_backward_fp32(
        float* wte_grad, float* wpe_grad,
        const float* dY,
        const int* X,
        int B, int T, int C,
        cudaStream_t stream );

    // FP16 host-launchable kernels
    void cuda_rope_forward_fp16(
        half* Y,
        const int* X,
        const half* wte, const half* wpe,
        int B, int T, int C,
        cudaStream_t stream );

    void cuda_rope_backward_fp16(
        half* wte_grad, half* wpe_grad,
        const half* dY,
        const int* X,
        int B, int T, int C,
        cudaStream_t stream );
}