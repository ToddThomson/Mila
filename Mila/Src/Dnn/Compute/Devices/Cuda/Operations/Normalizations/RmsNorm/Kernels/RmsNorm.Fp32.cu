...
{
    // FP32 RMSNorm forward kernel (per-slice strided access).
        ...
        int num_slices, int norm_dim, int inner_size, float epsilon )
    {
        ...
        // Compute mean of squares: m2 = (1/N) * sum_i x_i^2
        float m2 = 0.0f;
        int count = 0;
        ...
        // Broadcast final m2 to lane 0
        m2 = __shfl_sync( 0xffffffff, m2, 0 );

        // Convert to mean and compute reciprocal std: rstd = 1/sqrt(m2/N + eps)
        float inv_n = 1.0f / static_cast<float>( norm_dim );
        float rstd_val = rsqrtf( m2 * inv_n + epsilon );

#ifndef NDEBUG
        // Debug sanity check (cheap).
        ...
        // Write output: y = (x * rstd) * weight + bias  (strided)
        for ( int i = lane_id; i < norm_dim; i += WARP_SIZE )
        {
    ...
    // FP32 RMSNorm backward kernel (strided accesses).
    // Computes dX and accumulates dweight/dbias (atomicAdds).
        ...
        int num_slices, int norm_dim, int inner_size )
    {
        ...
        // g_i = dy_i * weight_i
        // accumulate sum_gx = sum_i g_i * x_i  and optionally dweight/dbias
        float sum_gx = 0.0f;
        ...
        for ( int i = lane_id; i < norm_dim; i += WARP_SIZE )
        {
            ...
            if ( dweight )
            {
                // dweight_i += dy_i * xhat_i  where xhat = x * rstd
                atomicAdd( &dweight[ i ], dy_val * ( x_val * rstd_val ) );
            }
        ...
        for ( int i = lane_id; i < norm_dim; i += WARP_SIZE )
        {
            ...
            // dx = rstd * g - x * rstd^3 * (1/N * sum(g * x))
            float out_dx = rstd_val * g - x_val * ( rstd_val * rstd_val * rstd_val ) * ( inv_n * sum_gx );

            // accumulate into dinp (atomic to be conservative)
