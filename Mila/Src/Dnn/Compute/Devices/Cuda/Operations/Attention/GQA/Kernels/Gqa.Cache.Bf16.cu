...
namespace Mila::Dnn::Compute::Cuda::Gqa
{
     ...
     *
     * BF16 equivalent of kvcache_write_q_fp32_kernel. Each thread handles
     * one scalar element. The source is a contiguous row-major Q tensor;
     ...
     */
    __global__ void kvcache_write_q_bf16_kernel(
        __nv_bfloat16* Q,
     ...
     *
     * BF16 equivalent of kvcache_write_kv_fp32_kernel. Each thread handles
     * one scalar element for both K and V simultaneously. The destination is
     ...
     * @param T_max     Maximum sequence length (KV cache capacity).
     */
    __global__ void kvcache_write_kv_bf16_kernel(
    ...
    __global__ void kvcache_expand_kv_bf16_kernel(
        ...
        cudaStream_t stream )
    {
        ...
        kvcache_write_q_bf16_kernel << < grid_size, block_size, 0, stream >> > (
            Q, X,
            batch, chunk_len,
            NH, HS,
        ...
        cudaStream_t stream )
    {
        ...
        kvcache_write_kv_bf16_kernel << < grid_size, block_size, 0, stream >> > (
            K, V, Xk, Xv,
            batch, chunk_len,
            NKV, HS,
            start_pos, max_seq_len);
        ...
        cudaStream_t stream )
    {
        ...
        kvcache_expand_kv_bf16_kernel << < num_blocks, block_size, 0, stream >> > (
            k_exp, v_exp,
            k_compact, v_compact,
