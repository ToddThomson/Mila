/**
 * @file TernaryOperation.ixx
 * @brief Abstract device-agnostic ternary operation interface.
 *
 * Typed, device-aware interface for operations with three inputs -> one output.
 */
...
export module Compute.TernaryOperation;
...
namespace Mila::Dnn::Compute
{
     ...
     * @tparam TInputA Element type for first input (defaults to TPrecision)
     * @tparam TInputB Element type for second input (defaults to TPrecision)
     * @tparam TInputC Element type for third input (defaults to TPrecision)
     *
    ...
    export template <
        ...
        TensorDataType TInputC = TPrecision>
        requires PrecisionSupportedOnDevice<TPrecision, TDeviceType>
    class TernaryOperation : public Operation<TDeviceType, TPrecision>
    {
    public:
        ...
        using TensorAType = Tensor<TInputA, MR>;
        using TensorBType = Tensor<TInputB, MR>;
        using TensorCType = Tensor<TInputC, MR>;
        ...
        virtual ~TernaryOperation() = default;

        /**
         * @brief Forward pass: output = f(input_a, input_b, input_c)
         *
         ...
         * @param input_c Third input from forward pass
         ...
         * @param input_c_grad Gradient w.r.t. third input (dL/dInputC)
         */
        virtual void backward(
            ...
            ITensor& input_b_grad,
            ITensor& input_c_grad ) const = 0;

    protected:
        // Typed dynamic-cast helpers to avoid manual rawData() casts.
        static const TensorAType& asATensor( const ITensor& t )
        {
            return dynamic_cast<const TensorAType&>(t);
        }

        static const TensorBType& asBTensor( const ITensor& t )
        {
            return dynamic_cast<const TensorBType&>(t);
        }

        static const TensorCType& asCTensor( const ITensor& t )
        {
            return dynamic_cast<const TensorCType&>(t);
        }
