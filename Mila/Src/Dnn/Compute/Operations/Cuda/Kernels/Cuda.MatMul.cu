...
#include "Cuda.Utils.h"
  ...
  *
  * This kernel computes the matrix multiplication C = A * B + bias without using vector operations,
  ...
  *
  * @param[out] out Pointer to the output matrix of shape [B*TDataType, OC]
  * @param[in] inp Pointer to the input matrix of shape [B*TDataType, C]
  ...
  */
__global__ void matmul_forward_kernel_scalar(
    float* out, const float* inp, const float* weight, const float* bias, int C, int OC ) {

    // Calculate indices
    int row = blockIdx.x * blockDim.x + threadIdx.x; // B*TDataType dimension
    ...
    if ( row < gridDim.x * blockDim.x && col < OC ) {
        ...
        for ( int i = 0; i < C; ++i ) {
            sum += inp[ row * C + i ] * weight[ col * C + i ];
            //sum += inp[ row * C + i ] * weight[ i * OC + col ];
        }
 ...
 *
 * This kernel computes the matrix multiplication C = A * B + bias, where:
 * - A is the input matrix of shape [128 * blockIdx.x, C]
 * - B is the weight matrix of shape [C, 128 * blockIdx.y]
 * - C is the output matrix of shape [128 * blockIdx.x, 128 * blockIdx.y]
 ...
 * @param[in] inp Pointer to the input matrix
 ...
 *
 * @param[out] out Pointer to the output matrix (B, TDataType, OC)
 * @param[in] inp Pointer to the input matrix (B, TDataType, C)
 ...
 * @param[in] TDataType Sequence length
...
void cuda_matmul_forward(
    ...
    if ( C % 4 != 0 || OC % 4 != 0 ||
        ...
        matmul_forward_kernel_scalar <<<gridDim, blockDim, 0, stream>>> (out, inp, weight, bias, C, OC);
    }
    else {
        // REF: ceil_div is (M + N - 1) / N
        
        // Use optimized kernel for aligned dimensions
        dim3 gridDim( ceil_div( B * T, 8 * sqrt_block_size ), ceil_div( OC, 8 * sqrt_block_size ) );
        dim3 blockDim( sqrt_block_size, sqrt_block_size );

        matmul_forward_kernel4 <<<gridDim, blockDim, 0, stream>>> (out, inp, weight, bias, C, OC);
    }
