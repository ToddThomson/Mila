...
//public:
    ...
    // TODO: Should fix alignment issues
    // 
    //void cuda_matmul_forward( float* out,
    //    const float* inp, const float* weight, const float* bias,
    //    int B, int T, int C, int OC ) {
    ...
    //    if ( C % 4 != 0 || OC % 4 != 0 ) {
    //        // Use a safe implementation for odd dimensions
    //        int sqrt_block_size = 16;
    //        dim3 gridDim( ceil_div( B * T, sqrt_block_size ), ceil_div( OC, sqrt_block_size ) );
    //        dim3 blockDim( sqrt_block_size, sqrt_block_size );

    //        // Use an alternative scalar kernel that doesn't use float4
    //        matmul_forward_kernel_scalar << <gridDim, blockDim >> > (out, inp, weight, bias, C, OC);
    //    }
    //    else {
