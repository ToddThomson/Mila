...
namespace Mila::Dnn
{
     ...
     * @return AxisPartition containing normalized axis and sizes.
     ...
     */
    export int64_t computeNumElements( const shape_t& shape )
    {
    ...
    {
        const int64_t actual_size = computeNumElements( shape );
    ...
    /**
     * @brief Multi-axis partition for normalization over trailing dimensions.
     */
    export struct MultiAxisPartition
    {
        ...
        shape_t outer_shape;      ///< Shape of outer dimensions
        shape_t normalized_shape; ///< Shape of normalized dimensions
    };

    /**
     * @brief Compute partition for normalization over trailing dimensions.
     *
     * Verifies that the trailing dimensions of @p shape match @p normalized_shape exactly,
     * then computes outer and normalized sizes and shapes.
     *
     * @param shape            Input tensor shape.
     * @param normalized_shape Expected trailing dimensions to normalize over.
     * @param op_name          Operation name for error messages.
     * @return MultiAxisPartition containing partition information.
    ...
    )
    {
        ...
        {
            outer_size *= shape[i];
            outer_shape.dims[outer_shape.ndim++] = shape[i];
        }
