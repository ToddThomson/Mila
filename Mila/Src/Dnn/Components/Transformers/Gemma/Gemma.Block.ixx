 ...
 * no training forward/backward. The (1 + weight) RMSNorm convention is applied by the
 * converter (it adds 1.0 to every norm weight at load), so the standard RmsNorm kernel is used.
 *
...
namespace Mila::Dnn
{
    ...
    public:
        ...
        TensorType& prefill( const TensorType& input, int position_offset ) override
        {
            ...
            {
                auto v_view = v_->view( shape_t{ B, T, NKV * HD }, 0 );
        ...
        TensorType& decode( const TensorType& input, int position ) override
        {
            ...
            {
                auto v_view = v_->view( shape_t{ B, 1, NKV * HD }, 0 );
