...
# Llama 3.1 8B Instruct — FP4 for Mila

**Built with Llama.**

A pre-quantized [Mila](https://github.com/ToddThomson/Mila) artifact of
`meta-llama/Llama-3.1-8B-Instruct`, in safetensors format.

**5.41 GiB**, down from 14.96 GiB at BF16. The FP4 packing is done once here instead
of on every load, so a Mila session starts near-instantly rather than quantizing 8 billion
parameters first. It fits a 12 GB consumer card with room for a long context.

## Files

| File | Purpose |
|---|---|
| `llama31_8b_instruct_fp4.safetensors` | Weights: packed FP4 E2M1 with per-group FP32 scales |
...
resolver.pull( "Llama-3.1-8B-Instruct-fp4", std::string( kDefaultHubOwner ) );

// Load from the store thereafter -- no network, and no manifest fetch.
const auto model = store.locate( "Llama-3.1-8B-Instruct-fp4" );
```
...
**The token embedding and `lm_head` are not quantized, and not tied** — both stay BF16, and
together they are around 2 GiB of the total. That is why this file is larger than a quarter of the
BF16 build rather than equal to it. Mila's Gemma chassis quantizes and ties both; the Llama chassis
does not yet, so the saving is available but unclaimed.
...
Quantization is lossy. This build trades some accuracy for fitting an 8B model on a 12 GB card,
which is the hardware Mila targets; if you need the weights untouched, convert them yourself from
`meta-llama/Llama-3.1-8B-Instruct` with the converters in the Mila repository.
...
Use is subject to the [Llama 3.1 Acceptable Use Policy](https://llama.meta.com/llama3_1/use-policy),
incorporated into the agreement by reference.

Base model: `meta-llama/Llama-3.1-8B-Instruct`, © Meta Platforms, Inc. This repository is not
...
attribution strings and different Acceptable Use Policies. A 3.2 derivative does not inherit this
