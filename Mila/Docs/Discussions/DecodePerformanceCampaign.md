...
- **Profile before believing "it's at the floor."** SM% above DRAM% on a streaming kernel is
  a smoking gun, and a same-run reference kernel (our FP8 lm_head at 97%) turns suspicion
  into proof.
- **Storage format is not compute format.** FP8 KV needs no Hopper; FP4 weights need no
  Blackwell. Ada dequants both inline at speed. The hardware ladder gates *arithmetic*, not
  *bytes*.
- **Look for identity copies at T=1.** Layout permutes that do real work on a sequence
  degenerate to memcpys on a single token — ours had been running twice per layer per token.
- **Poison what must not be read.** A NaN-filled dead zone converts "the kernel should only
  read the live band" from a code-review claim into a test failure.
- **Pay the tuition once.** Every hour of Part 1's oracle-and-sanitizer discipline came back
...
*Kernels: `Gqa.Decode.Bf16.cu`, `CudaMatVecBias.Bf16.cu`. Ideas they stand on: FlashAttention
and FlashAttention-2 (Dao et al.), Flash-Decoding (Dao, Haziza, Massa, Sizov), and the online
softmax normalizer (Milakov & Gimelshein) — full citations in
[ATTRIBUTIONS.md](https://github.com/ToddThomson/Mila/blob/master/ATTRIBUTIONS.md).*
