...
namespace Bard
{
    ...
    void train( const BardConfig& config )
    {
        ...
        {
            throw std::runtime_error( "Data directory does not exist: " + data_dir_path.string() );
        }

        if ( !fs::exists( encoded_file ) )
        {
            throw std::runtime_error( "Encoded data file not found: " + encoded_file.string() );
        }
