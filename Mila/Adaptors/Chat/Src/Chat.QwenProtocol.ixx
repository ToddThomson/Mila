 ...
 * @brief Qwen 3.8's ChatML turn structure, reasoning gate and tool-call grammar.
 *
...
export module Chat.QwenProtocol;

import Chat.Message;
import Chat.SystemPrompt;
import nlohmann.json;

namespace Mila::ChatApp::Qwen
{
    ...
    inline constexpr std::string_view kTurnOpen = "<|im_start|>";
    inline constexpr std::string_view kTurnClose = "<|im_end|>";
    ...
    inline constexpr std::string_view kToolCallClose = "</tool_call>";
    inline constexpr std::string_view kToolResponseOpen = "<tool_response>";
    inline constexpr std::string_view kToolResponseClose = "</tool_response>";
    ...
    /**
     * @brief Map Chat's 1..5 effort scale onto the three levels the model knows.
     *
     * The scale is the harness's, shared across families, so the ends are pinned to the ends
     * and the middle to the silent default.
     */
     ...
     *        them inside <tools>.
     */
    inline std::string toolsSection( const std::string& signatures )
    {
        return std::format(
     ...
     */
    inline std::string formatToolCall( const std::string& name, const std::string& arguments )
    {
        std::string rendered;
     ...
     * Chat's MessageRole::Tool renders as "ipython" for Llama, which would open a role Qwen was
     * never trained to read. Consecutive results are not merged into one user turn the way the
     * template does; the harness dispatches one call per round, so the case does not arise.
     */
    inline std::string formatTurn( const ChatMessage& message )
    {
        std::string turn;
        ...
        if ( message.role == MessageRole::Assistant )
        {
            ...
            // the turn's own reasoning here; Chat never keeps it -- ChannelParser splits it off
            // and only the answer is stored -- and the span still has to be present, because its
            // absence is what tells the model this turn was generated with thinking off.
        ...
        {
            turn += message.role == MessageRole::System ? "system\n" : "user\n";
            turn += message.content;
        }
     ...
     * orders it -- reasoning instruction, then tools, then whatever the user configured -- and
     ...
     *        carries a closing marker with no opening one -- see ChannelDelimiters.
     *
     * @param tool_signatures One JSON object per tool, newline separated. Empty omits the whole
     *        tools section, which is what tells the model there are none.
     *
        ...
        const std::string& tool_signatures = {} )
    {
        ...
        {
            prompt += formatTurn( { MessageRole::System, system_content } );
        }
     ...
     */
    export inline std::optional<ToolCall> parseToolCall( const std::string& response )
    {
        ...
        // Correlates the call with the Tool turn carrying its result. Qwen's template pairs them
        // positionally and never renders an id, so this exists for Chat's own history.
        static std::atomic<std::uint32_t> next_id{ 1 };

        call.id = std::format( "call_{}", next_id.fetch_add( 1, std::memory_order_relaxed ) );
