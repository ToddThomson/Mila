...
| `:Random` | `fill_normal` `fill_uniform` `xavier` | `[x]` **done** — the FP32-only CUDA bug this row used to defer behind is fixed, so the numeric tests it warned against are now the point. `Random.Cuda.cpp` sweeps FP32 + BF16 over distribution shape, uniform bounds, the Glorot bound, the odd-count Box-Muller padding path, empty-tensor no-ops, and a neighbouring-allocation check for the overrun. Seeded via `RandomGenerator::setSeed` so a distribution failure repeats rather than flickers |
