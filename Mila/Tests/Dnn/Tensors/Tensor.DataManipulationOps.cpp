...
namespace Tensors::Tests
{
    ...
    TEST( TensorDataManipulationTest, Fill_HostTensor ) {
    ...
    TEST( TensorDataManipulationTest, Fill_CudaTensor ) {
        std::vector<size_t> shape = { 2, 3 };
        Tensor<float, Compute::CudaMemoryResource> dev_tensor( shape );
        dev_tensor.fill( 3.1415f );

        auto cpu_tensor = dev_tensor.toHost<Compute::HostMemoryResource>();
    ...
    TEST( TensorDataManipulationTest, Fill_DifferentDataTypes ) {
        ...
        // Test with uint16_t
        {
            ...
            uint16_t fill_value = 1000;
            uint16_tensor.fill( fill_value );

            for ( size_t i = 0; i < shape[ 0 ]; ++i ) {
                for ( size_t j = 0; j < shape[ 1 ]; ++j ) {
                    EXPECT_EQ( (uint16_tensor[ i, j ]), fill_value );
                }
            }
        }
        ...
        // Test with int16_t
        {
            ...
            int16_t fill_value = -500;
            int16_tensor.fill( fill_value );

            for ( size_t i = 0; i < shape[ 0 ]; ++i ) {
                for ( size_t j = 0; j < shape[ 1 ]; ++j ) {
                    EXPECT_EQ( (int16_tensor[ i, j ]), fill_value );
                }
            }
        }
    }

    TEST( TensorDataManipulationTest, Fill_PinnedMemory ) {
        std::vector<size_t> shape = { 2, 2 };
        Tensor<float, Compute::CudaPinnedMemoryResource> pinned_tensor( shape );
        pinned_tensor.fill( 2.718f );

        for ( size_t i = 0; i < shape[ 0 ]; ++i ) {
            for ( size_t j = 0; j < shape[ 1 ]; ++j ) {
                EXPECT_FLOAT_EQ( (pinned_tensor[ i, j ]), 2.718f );
            }
        }
    }

    TEST( TensorDataManipulationTest, Fill_ManagedMemory ) {
        std::vector<size_t> shape = { 3, 3 };
        Tensor<float, Compute::CudaManagedMemoryResource> managed_tensor( shape );
        managed_tensor.fill( 1.414f );

        for ( size_t i = 0; i < shape[ 0 ]; ++i ) {
            for ( size_t j = 0; j < shape[ 1 ]; ++j ) {
                EXPECT_FLOAT_EQ( (managed_tensor[ i, j ]), 1.414f );
            }
        }
    }

    TEST( TensorDataManipulationTest, Fill_EmptyTensor ) {
        Tensor<float, Compute::HostMemoryResource> empty_tensor;
        EXPECT_TRUE( empty_tensor.empty() );

        EXPECT_NO_THROW( empty_tensor.fill( 1.0f ) );
        EXPECT_TRUE( empty_tensor.empty() );
    }

    TEST( TensorDataManipulationTest, Fill_SingleElement ) {
        Tensor<float, Compute::HostMemoryResource> single_tensor( { 1 }, 0.0f );
        single_tensor.fill( 99.9f );

        EXPECT_FLOAT_EQ( single_tensor[ 0 ], 99.9f );
    }

    TEST( TensorDataManipulationTest, Fill_LargeTensor ) {
        std::vector<size_t> large_shape = { 100, 50 };
        Tensor<float, Compute::HostMemoryResource> large_tensor( large_shape );
        large_tensor.fill( 7.777f );

        // Spot check various locations
        EXPECT_FLOAT_EQ( (large_tensor[ 0, 0 ]), 7.777f );
        EXPECT_FLOAT_EQ( (large_tensor[ 50, 25 ]), 7.777f );
        EXPECT_FLOAT_EQ( (large_tensor[ 99, 49 ]), 7.777f );
    }
