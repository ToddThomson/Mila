...
namespace Dnn::Compute::Tests
{
	...
	TEST_F( DeviceTypeTest, deviceTypeToString_InvalidType )
	{
		...
		EXPECT_THROW( deviceTypeToString( invalid_type ), std::invalid_argument );
	}
	...
	TEST_F( DeviceTypeTest, deviceTypeToString_ErrorMessage )
	{
		...
		catch ( const std::invalid_argument& e )
		{
			...
			EXPECT_NE( error_msg.find( "Invalid DeviceType" ), std::string::npos );
		}
	}
	...
	TEST_F( DeviceTypeTest, ToDeviceType_InvalidStrings )
	{
		...
		EXPECT_THROW( toDeviceType( "" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "INVALID" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "GPU" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "DEVICE" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "123" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "AUTO" ), std::invalid_argument );
	}

	TEST_F( DeviceTypeTest, ToDeviceType_WhitespaceHandling )
	{
		// Test strings with whitespace (should fail - no trimming implemented)
		EXPECT_THROW( toDeviceType( " CPU" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "CPU " ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( " CPU " ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "C P U" ), std::invalid_argument );
	}
	...
	TEST_F( DeviceTypeTest, ToDeviceType_SpecialCharacters )
	{
		...
		EXPECT_THROW( toDeviceType( "CPU\t" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "CPU-1" ), std::invalid_argument );
		EXPECT_THROW( toDeviceType( "CPU:0" ), std::invalid_argument );
	}
	...
	TEST_F( DeviceTypeTest, ErrorMessage_ToDeviceType )
	{
		...
		{
			std::string error_msg = e.what();
			// Verify error message contains the offending token (uppercased) and the phrase "Invalid device type"
			EXPECT_NE( error_msg.find( "INVALID" ), std::string::npos );
			EXPECT_NE( error_msg.find( "Invalid device type" ), std::string::npos );
		}
	}
	...
	TEST_F( DeviceTypeTest, RoundTripConversion_CaseInsensitive )
	{
		...
		for ( const auto& [input, expected_type] : test_cases )
		{
			...
			std::string canonical = deviceTypeToString( parsed_type );
	...
	TEST_F( DeviceTypeTest, LargeStringHandling )
	{
		...
		EXPECT_THROW( toDeviceType( long_string ), std::invalid_argument );
	}
	...
	TEST_F( DeviceTypeTest, StringWithNullCharacters )
	{
		...
		// Should only process up to first null character, which should succeed for Device::Cpu()
		EXPECT_EQ( toDeviceType( "CPU" ), DeviceType::Cpu );

		// But string with null in middle should fail
		std::string middle_null = std::string( "C\0PU", 4 );
		EXPECT_THROW( toDeviceType( middle_null ), std::invalid_argument );
	}
