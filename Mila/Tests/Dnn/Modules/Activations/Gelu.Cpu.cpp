...
namespace Modules::Activations::Tests
{
    ...
    struct GeluCpuTestData {
        ...
        std::shared_ptr<Gelu<DeviceType::Cpu>> gelu_module;
        ...
        static GeluCpuTestData CreateWithDeviceId( int device_id, size_t batch, size_t seq, size_t chan ) {
            ...
            d.gelu_module = std::make_shared<Gelu<DeviceType::Cpu>>( device_id, config );
        ...
        static GeluCpuTestData CreateWithExecutionContext( std::shared_ptr<ExecutionContext<DeviceType::Cpu>> ctx, size_t batch, size_t seq, size_t chan ) {
            ...
            d.gelu_module = std::make_shared<Gelu<DeviceType::Cpu>>( ctx, config );
    ...
    TEST_F( GeluCpuTests, Construct_WithDeviceId_ThrowsWhenOpNotRegistered ) {
        // Negative case: when the backend is not registered the constructor must throw.
        if (!isOperationRegistered<DeviceType::Cpu, TensorDataType::FP32>( "Cpu::GeluOp" ))
        {
            EXPECT_THROW( (Gelu<DeviceType::Cpu>( -1, GeluConfig() )), std::runtime_error );
        }
        else
        {
            // If registered, construction must succeed.
            EXPECT_NO_THROW( (Gelu<DeviceType::Cpu>( -1, GeluConfig() )) );
        }
    }
    ...
    TEST_F( GeluCpuTests, Construct_WithExecutionContext_ThrowsWhenOpNotRegistered ) {
        ...
        if (!isOperationRegistered<DeviceType::Cpu, TensorDataType::FP32>( "Cpu::GeluOp" ))
        {
            EXPECT_THROW( (Gelu<DeviceType::Cpu>( ctx, GeluConfig() )), std::runtime_error );
        }
        else
        {
            EXPECT_NO_THROW( (Gelu<DeviceType::Cpu>( ctx, GeluConfig() )) );
        }
    }

    TEST_F( GeluCpuTests, ParameterCount_IsZeroOrConstructorThrows ) {
        if (!isOperationRegistered<DeviceType::Cpu, TensorDataType::FP32>( "Cpu::GeluOp" ))
        {
            EXPECT_THROW( (Gelu<DeviceType::Cpu>( -1, GeluConfig() )), std::runtime_error );
            return;
