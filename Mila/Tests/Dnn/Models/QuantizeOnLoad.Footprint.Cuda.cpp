...
namespace Mila::Tests::Dnn::Models
{
    ...
    namespace
    {
        ...
        LoadMeasurement measureLoad( const fs::path& weights, TConfig config )
        {
...

            {
                ...
                measurement.consumed_after_load = free_before - freeDeviceBytes();
                ...
                measurement.consumed_after_generation = free_before - freeDeviceBytes();
            }
            ...
            const std::string& label, const fs::path& bf16_weights, const fs::path& exported_weights, TConfig config )
        {
            ...
            const LoadMeasurement exported = measureLoad<TModel>( exported_weights, config );
            const LoadMeasurement fitted = measureLoad<TModel>( bf16_weights, config );
