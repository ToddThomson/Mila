...
Hardware for model development and validation. **24 GB is the minimum that makes the next models
runnable; 32 GB on Blackwell also reaches the native FP4 compute path Mila cannot test today.**
...
- **RTX 5090 (32 GB, sm_120)** — the headline ask, because it closes the capacity gap and the sm_120
  ...
  current rig** and the design is forward-compatible on paper only. The 32 GB also fits the
  sparse-MoE class — Gemma 26B-A4B, Qwen3-30B-A3B, gpt-oss-20b at FP4 — and an unquantized BF16 12B
  quality oracle on a single card, none of which fit the 12 GB ceiling.
- **24 GB (RTX 4090, or any 24 GB card)** — the floor. It runs both named targets at FP4 with room
  for context, which is what the 12 GB card cannot do at all. It leaves the sm_120 gap open.
...
- **Multi-GPU** — tensor-parallel paths untestable on a single card today. Consumer Blackwell has no
  NVLink, so a 2x 5090 pair tests tensor parallelism over PCIe — which is the configuration a
  desktop system would use.
...
- **DGX Spark (GB10, 128 GB coherent, sm_121)** — Mila has never been built or run on **aarch64**;
  ...
  GB10 is one coherent pool with nothing to copy into. Explicitly **not** a performance machine — at
