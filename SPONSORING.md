...
Mila accepts **compute and hardware sponsorship only — never money.**
...
- **Blackwell (sm_120)** — native FP4 on current silicon (partly self-funded via an RTX 5060 Ti).
...
For compute or hardware collaboration, contact **`<sponsorship contact — e.g. sponsors@toddt.me>`**.

Compute sponsors are acknowledged, with thanks, once they come aboard — there is no pre-populated
sponsor wall.
