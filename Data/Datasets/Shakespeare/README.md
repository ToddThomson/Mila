# tiny_shakespeare dataset — organization and preprocessing for CharLM

This document explains the on-disk layout expected by the CharLM sample and how to preprocess the Tiny Shakespeare text using the Mila tokenizer tool (tokenizer.exe) so the CharLM sample can load the data and vocabulary.

## Directory layout (expected)
Place the dataset under `Data/Datasets/tiny_shakespeare/` with the following recommended files:

- `input.txt`
  - Raw text corpus (one file containing the training text). This is the path the CharLM sample expects by default.
- `input.txt.vocab`
  - Binary vocabulary file produced by the Mila tokenizer. CharLM looks for `<data_file>.vocab` (where `data_file` is the path you pass to the sample).
...
Note: filenames above are examples. The CharLM sample uses the `--data-file` path you pass; it expects a matching `.vocab` file at `<data-file>.vocab`.
...
The CharLM sample requires at minimum the `.vocab` file to be present. If only raw `input.txt` and `input.txt.vocab` exist, the data loader may perform in-memory encoding as needed; providing a tokenized binary file improves throughput.
...
- `--tokenizer-type` — `char` for the CharLM pipeline (single-byte character mapping).
...
If your project includes a tokenizer implementation under `Samples/CharLM/Tools` or a build target that produces `tokenizer.exe`, build that target first (via the usual CMake/Ninja build process) and run the produced binary.

## Running the CharLM sample
After producing `input.txt.vocab` (and optionally the tokenized `.mila` file), build the CharLM sample and run:
CharLM.exe --data-file Data\Datasets\tiny_shakespeare\input.txt 
...
- If you created a tokenized binary (`.mila`), ensure the dataset reader/loader for CharLM is configured to use it; otherwise the loader will fall back to encoding/reading from the raw text using the `.vocab`.
