...
- [ ] FIXME/TODO debt triage — the source carries ~71 `FIXME` + ~69 `REVIEW` + ~25 `TODO` (165 total); `FIXME` reads as "known broken", and several are commented-out core paths (weight initializers bypassed as "takes too long"; commented `prefill`/`xavier`/`normal` calls). Fix the real ones, convert the rest to tracked GitHub issues, do not ship literal "FIXME"s in public source. Distinct from the "debug instrumentation gated/removed" gate (the `std::cout` 12 files / `std::cerr` 5 / `printf` 6 usage)
