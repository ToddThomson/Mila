...
- [ ] The docs job is decoupled from the build — Doxygen is a source parser (`EXTRACT_ALL` reads `.ixx`/`.cuh` directly) and in principle needs no compiled library, no GPU, no clang. PARTIALLY DONE 2026-06-09: the old build-tree-download / foreign-CMakeCache coupling is gone — docs is its own workflow that runs Doxygen via `cmake --build --target docs` (compiling nothing). REMAINING: the job still runs CMake **configure inside the CUDA container** because the `docs` target is CMake-registered and the root configure requires the CUDA toolkit, so it is not yet a pure checkout -> doxygen -> publish job. Full decoupling means driving Doxygen without a CUDA-dependent configure (standalone Doxyfile, or a CUDA-free docs-only configure path)
- [x] Publish via Actions-native Pages (`actions/upload-pages-artifact` + `actions/deploy-pages`) rather than the `gh-pages` orphan branch + `JamesIves` action — DONE 2026-06-09. The `JamesIves` action was silently failing inside the CUDA build container (`git config` -> "fatal: not in a git directory") and had left the published site frozen ~12 months. `docs.yml` rewritten to a build job (Doxygen in the container) -> `upload-pages-artifact`, plus a separate deploy job on the bare host -> `deploy-pages` (git-free API publish). Repo Pages Source switched to "GitHub Actions" (`build_type=workflow`); `workflow_dispatch` added for manual re-publish. Validated green on the PR #14 merge; site live and current. `gh-pages` branch now orphaned (safe to delete)
...
- [ ] Bump the docs Doxygen version — `docs.yml` installs Doxygen via apt, which on Ubuntu 26.04 pins **1.15** while latest is **1.17**; C++23 module rendering fidelity (above) improves across Doxygen releases, so install 1.17 from the upstream release tarball rather than apt. Pairs with the module-rendering verification
...
- [x] GPU test honesty (highest priority) — RESOLVED 2026-06-07 by the CI refactor. The old separate `test` job ran on a bare runner with no GPU and could false-green by silently skipping CUDA tests; that job was removed. CI is now **GPU-free by design** — a single `Compile (clang-21) + packaging gates` job that compiles the tree and runs only the GPU-free packaging gates; the workflow header states this explicitly. GPU correctness is validated locally on real hardware (the full suite is run before committing), not in CI, so the badge means "compiles cross-compiler + packaging-consumable" with no hidden device-test skips. A future self-hosted GPU runner remains optional, not a trust gate
- [x] Stop passing the whole configured `build/` tree between jobs — DONE 2026-06-07. The refactor consolidated to a single job, eliminating the gigabytes-large, environment-specific cross-runner `build/` handoff entirely; `docs` is its own separate workflow now. No CMakeCache full of foreign absolute paths is shipped between runners anymore
- [ ] CI/CD pipeline efficiency pass (deferred to a later beta phase — the pipeline is correct and green; this is optimization, not a trust gate). Measured master run: Build ~18.5 min + packaging gates ~25 min = ~44 min; dev pushes ~20 min (the packaging gates are master-bound). The dominant cost is **C++23 module compilation, not the `.cu` kernels** — so the ccache launcher currently wired CUDA-only misses the real bottleneck (its cross-run cache-restore is effectively a no-op, and ccache was deliberately kept off the C++ module units as unreliable). The structural waste is that the same module tree is compiled up to **three times per master run**: the Build, the `find_package` gate (recompiles the exported module interfaces), and the FetchContent gate (rebuilds the whole tree in-tree from scratch). Levers, ranked by value: (1) module-aware caching — spike whether clang-21 + ccache 4.x can reliably cache the module compiles (BMIs are compiler/path-sensitive, hence the original exclusion) — highest value, hardest, and the only one that hits the bottleneck; (2) `-O0`/Debug in CI since the binaries are never run, but module BMI generation is front-end work `-O0` does not reduce, so the win is partial (codegen on tests/samples/`.cu` only); (3) move the FetchContent full-rebuild gate to tag-only if release cadence rises. Deeper, source-side lever (out of CI scope): the module dependency-graph critical path caps `-j` parallelism regardless of core count
...
- [x] Formatter/linter config (highest-ROI scaffolding) — DONE 2026-06-09. Added `.editorconfig` (VS 2026-generated C++ `cpp_*` keys, the Windows-authoritative formatter; fixed the non-spec `indent_style = spaces` -> `space` and added the omitted universal hygiene keys: `charset`, `trim_trailing_whitespace`, `insert_final_newline`), `.clang-format` (best-effort match for non-VS contributors — `ColumnLimit: 0` to avoid reflow churn, Allman + spaces-in-parens; header documents the rules it cannot express), and `.clang-tidy` (conservative clangd aid). Conscious defers recorded in the file headers: `.clang-tidy` is NOT a CI gate yet (needs a module-aware `compile_commands.json`; clang-tidy C++23-module support is immature — same blocker as the IWYU work in Module Hygiene), and `.clang-format` cannot express the no-abbreviation naming rule, the blank-line-before-control-flow rule, or ASCII-only comments (these stay review-time conventions, cross-referenced in CONTRIBUTING.md). clang-format vs VS will differ on expression-paren spacing (clang-format cannot distinguish parameter parens from expression parens); documented rather than chased
- [x] GitHub community-health files — DONE 2026-06-09. Added `CODE_OF_CONDUCT.md` (self-authored, plain-worded; references the Contributor Covenant as the standard rather than reproducing its verbatim text), `SECURITY.md` (solo-maintainer private-disclosure policy via GitHub Security Advisories + email; scope points at the weight-blob/tokenizer parse paths), and `PULL_REQUEST_TEMPLATE.md` in the repo root (GitHub-recognized there); issue templates (`bug_report.md`, `feature_request.md`, `config.yml`) under `.github/ISSUE_TEMPLATE/` (GitHub's only recognized location). config.yml contact links point at the `dev` branch (correct during alpha; flip to `master` at the beta default-branch switch). Completes GitHub's community-standards checklist; pairs with the good-first-issue labels requirement
