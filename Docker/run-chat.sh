...
#
# The POST_BUILD step copies Data/ next to the binary in /build, and mila-chat now finds
# that directory itself: executable_directory() reads /proc/self/exe on Linux rather than
# falling through to the working directory. The cd below is therefore redundant and is
# kept only until a container run confirms it -- see Specifications/ChatConfiguration.md
# section 2, which retires this and the runtime image's WORKDIR together.
#
...
# Any argument (e.g. --settings Data/other.json, --help) is forwarded to mila-chat.
set -euo pipefail
